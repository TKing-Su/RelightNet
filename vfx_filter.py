...
try:
    ...
    DEFAULT_CONVERT_MODE = "robust_quantile"
    DEFAULT_DELETE_EXR_AFTER_CONVERT = False
    DEFAULT_EXR_CONVERT_CONFLICT = "skip"
    DEFAULT_FIXED_MAX = 1.0
    DEFAULT_FIXED_MIN = 0.0
    DEFAULT_HIGH_Q = 0.998
    DEFAULT_LOW_Q = 0.002
    batch_convert_depth_exr = None
...
def main() -> int:
    ...
    exr_results: List[ExrConvertResult] = []
    if exr_enabled and batch_convert_depth_exr is None:
        logger.warning("EXR -> PNG conversion skipped: exr2png.py is not available in this repository.")
        exr_enabled = False
    if exr_enabled:
